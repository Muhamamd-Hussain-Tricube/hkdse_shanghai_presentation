...
export const IELTS_SECTION_BLUEPRINT: SectionBlueprint[] = [
  { id: "R", title: "Paper 1 Reading", minutes: 90, marks: 20 },
  { id: "W", title: "Paper 2 Writing", minutes: 120, marks: 25 },
  { id: "L", title: "Paper 3 Listening & Integrated Skills", minutes: 120, marks: 30 },
  { id: "S", title: "Paper 4 Speaking", minutes: 20, marks: 10 },
];

const SECTION_IDS = IELTS_SECTION_BLUEPRINT.map((s) => s.id);

function buildSystemPrompt(ctx: GenerateContext) {
  const { grade, curriculum } = ctx;
  return `You are an HKDSE English Language practice test author. Generate a fresh practice paper for a candidate (background: ${grade}, ${curriculum}). This is NOT an official HKEAA paper--label content as practice.

ALL question text must be in English only.

Section R -- Paper 1 Reading (90 min, 20 marks):
  - Two original HKDSE-style texts about Hong Kong school or community life, not copied from past papers.
  - 10 questions total: mix of mcq and short_text. Include "answer" for objective/short items.
  - Put the texts on "passage" with clear Part A and Part B headings.

Section W -- Paper 2 Writing (120 min, 25 marks):
  - W1 (10 marks): type="short_text", guided short task around 200 words.
  - W2 (15 marks): type="essay", longer open-ended task around 400 words, with four essay_prompts options.
  - Include rubrics aligned to HKDSE English content, organisation, language and tone.

Section L -- Paper 3 Listening & Integrated Skills (120 min, 30 marks):
  - Set "listening_script" on the section: one realistic dialogue/announcement transcript (~550 words). Do not mention transcript practice.
  - Set "passage" on the section as a Data File with notes, email extracts or charts that students combine with the audio.
  - ~8 questions: mcq, short_text and one extended writing task requiring 100-200 words.
  - Include "answer" for all objective items.

Section S -- Paper 4 Speaking (20 min, 10 marks):
  - S1 (6 marks): type="short_text", group interaction task based on a short prompt.
  - S2 (4 marks): type="short_text", individual response to examiner follow-up questions.
  - Put polished sample response transcripts in "answer" so the demo recorder can autosave a realistic spoken response.
  - Include rubrics for communication strategies, ideas, pronunciation, fluency and interaction.

Question IDs: R1..R10, W1, W2, L1..L8, S1, S2.
Section marks must match: R=20, W=25, L=30, S=10.`;
}

function validatePayload(payload: { sections?: Array<{ id: string; marks?: number }> }) {
  return validatePayloadStrict(payload, "ielts_practice", IELTS_SECTION_BLUEPRINT);
}

export const ieltsPracticeBlueprint: TestBlueprint = {
  testType: "ielts_practice",
  sectionBlueprint: IELTS_SECTION_BLUEPRINT,
